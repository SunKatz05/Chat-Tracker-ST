        ...
        try {
            ...
            if (body.messages && Array.isArray(body.messages)) {
                ...
                if (context && typeof context.getTokenCount === 'function') {
                    ...
                    lastEstimate = context.getTokenCount(promptText);
...
async function handleFetchResponse(response, urlString) {
    try {
        ...
        if (usage) {
            ...
            const context = typeof SillyTavern !== 'undefined' ? SillyTavern.getContext() : null;
            const chatId = context?.chatId || context?.characterId || 'default';
            
            usagePerChat[chatId] = usage;
            localStorage.setItem('chatTracker_usage', JSON.stringify(usagePerChat));
    ...
    try {
        ...
        const handleRefreshEvent = (eventName, opts = {}) => {
            ...
            if (resetIntercepted) {
                ...
                const currentContext = typeof SillyTavern !== 'undefined' ? SillyTavern.getContext() : null;
                const chatId = currentContext?.chatId || currentContext?.characterId || 'default';
                lastUsage = usagePerChat[chatId] || { prompt: 0, completion: 0, total: 0 };
...
function loadState() {
    try {
        const savedUsage = localStorage.getItem('chatTracker_usage');
        if (savedUsage) {
            usagePerChat = JSON.parse(savedUsage);
        }
    } catch (e) {}
